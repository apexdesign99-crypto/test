...
| 壁量計算書 | 必要壁量（地震・風）・存在壁量・充足率・四分割法（令46条4項） |
...
住所を入れると、公的データから用途地域・建蔽率・容積率・前面道路・ハザードを引いて
`Site` を組み立てる。**取得できた項目には出典が、できなかった項目には既定値と警告が残る**ので、
どこまでが公的データでどこからが仮定かが常に分かる。出典はレポートと確認申請シートにも載る。

```bash
# 外部 API を使う（国土地理院・OpenStreetMap・ハザードマップポータル）
python3 -m ai_land_design resolve --address "東京都世田谷区代田1-1-1" --area 180 --live

# 全工程まで一気に実行（診断→図面→IFC→申請書）
python3 -m ai_land_design run --address "東京都世田谷区代田1-1-1" --area 180 --live --out out/
```

| 取得項目 | データソース | 備考 |
| --- | --- | --- |
| 緯度経度 | 国土地理院 住所検索API | 利用登録不要 |
| 用途地域・建蔽率・容積率 | 国土数値情報 A29（GeoJSON）／不動産情報ライブラリ API | A29 はオフラインで使える。API はキーが必要 |
| 浸水深・土砂災害警戒区域 | ハザードマップポータル（ラスタタイル） | タイルの配色を凡例に照合。目安値 |
| 前面道路の幅員・方位 | OpenStreetMap（Overpass API） | 幅員は width タグがある場合のみ |
| 既存建物の外形 | OpenStreetMap | 参考情報（敷地境界ではない） |

**敷地形状（筆界）は公開 API では取得できない。** 既定では敷地面積からの矩形近似を使い、
警告を出す。地積測量図の座標がある場合は `polygon` で渡す。

### API キーの登録

不動産情報ライブラリの API キーは、Web アプリの **データソース設定画面（`/settings`）** から登録できる。

```bash
uvicorn webapp.main:app --reload   # → http://127.0.0.1:8000/settings
```

- API キー、用途地域 API 名（既定 `XKT013`）、外部 API の利用可否、ローカルデータのパスを設定
- 「接続テスト」で各データソースへ実際に接続し、結果を項目ごとに表示（401/403 は「キーが無効か利用申請が未承認」と表示）
- 保存先は `.ai_land_design/settings.json`（`AI_LAND_DESIGN_CONFIG` で変更可）。**ファイルは 0600、`.gitignore` 済み**
- 画面・API がキーを返すことはない（末尾4文字以外はマスク）

> **このアプリに認証機能はない。** 設定画面を含め外部に公開せず、ローカルまたは社内ネットワーク限定で使うこと。

環境変数でも設定できる（画面の保存値が環境変数より優先される）。

```bash
export AI_LAND_DESIGN_LIVE=1                      # 外部 API を使う
export AI_LAND_DESIGN_ZONING_GEOJSON=A29-tokyo.geojson  # 国土数値情報 A29
export AI_LAND_DESIGN_GEOCODE_CACHE=.cache/geocode.json # 問い合わせ結果のキャッシュ
export AI_LAND_DESIGN_GEOCODE_TABLE=addresses.json      # 住所→座標のローカル辞書（オフライン）
export REINFOLIB_API_KEY=...                            # 不動産情報ライブラリ
```

外部 API に到達できない場合も落ちず、その項目を警告に落として処理を続ける。
...
画面左上の「住所から取得」で公的データを引いてフォームを埋められる（出典と要確認事項が並ぶ）。
画面は左が入力フォーム（住所取得・サンプル読込・敷地・用途地域・前面道路・ハザード・建築計画・申請情報）、
...
| `GET/PUT /api/settings` | データソース設定の取得・保存（API キーはマスクして返す） |
| `POST /api/settings/test` | 登録した設定で各データソースへ接続テスト |
| `DELETE /api/settings/api-key` | 保存済み API キーの削除 |
| `POST /api/resolve` | **住所から敷地条件を組み立てる**（出典・要確認つき。結果をそのまま `/api/analyze` に渡せる） |
...
**壁量計算（令46条4項）を実行する**（`structure.py`）。木造軸組構法が対象。

| 算定 | 内容 |
| --- | --- |
| 必要壁量（地震力） | 床面積 × 係数（屋根の重さ・階数で決まる表） |
| 必要壁量（風圧力） | 見付面積 × 50 cm/m²。見付面積は各階の床面から 1.35m 超の投影面積 |
| 存在壁量 | 外周壁（開口部を控除）と 0.91m 以上の間仕切壁 × 壁倍率 |
| 配置バランス | 四分割法（告示1352号）。側端 1/4 の充足率から壁率比を算定 |

判定は「壁量の充足」と「配置バランス」を分けて出す。間取り生成側でも、耐力壁を
残すため開口幅を壁長の半分までに抑える（採光 1/7 に必要な幅がそれを超える場合は採光を優先）。

> **係数表について**: 既定値は改正前（令和7年3月31日まで）の令46条4項 表2 です。
> 令和7年4月施行の改正で必要壁量の算定方法が見直されているため、**最新の告示値であることを
> 確認するまで判定は「要確認」**として返します。画面のチェックボックス、または
> `structure.SeismicTable` の差し替えで確定させてください。
...
| 単体規定 | 居室の採光 1/7（法28条1項・令19条）／換気 1/20（法28条2項）／天井高 2.1m（令21条）／階段の寸法（令23条）／**壁量と配置バランス（令46条4項・告示1352号）**／シックハウス（法28条の2） |
| 対象外 | 接合部（N値計算）・横架材・基礎・省エネ計算・日影図・天空率は「要確認」として明示 |
...
| GIS・地図 | `sources/gis.py` | 敷地 JSON の読み込みとモデル化 |
| 住所解決 | `sources/resolve.py` | 住所 → 敷地条件の組み立てと出典・警告の記録 |
| ジオコーディング | `sources/geocoding.py` | 国土地理院 API、キャッシュ、局所平面座標への変換 |
| 用途地域 | `sources/zoning_lookup.py` | 国土数値情報 A29 / 不動産情報ライブラリ API |
| ハザード | `sources/hazard_lookup.py`, `sources/tiles.py` | タイル画像の取得と PNG デコード、凡例照合 |
| 道路・建物 | `sources/parcel.py` | Overpass API から前面道路・既存建物を取得 |
| 設定の保存 | `webapp/settings_store.py` | API キー・参照パスの保存（0600・マスク・環境変数との優先順位） |
...
- 壁量計算は行うが、耐力壁の位置・仕様は「外周壁と 0.91m 以上の間仕切壁」を仮定した概算。接合部（N値計算）・横架材・基礎・地盤と、許容応力度計算は対象外
- 省エネ計算（外皮性能・一次エネルギー消費量）は行っていない
- 間取りは矩形の外形を前提とした自動生成で、動線・家具配置・意匠は検討していない
- 図面は下書きであり、通り芯・仕上・納まり・矩計図は含まれない
- 確認申請書は正規様式ではなく、様式に転記するためのデータシート
- 敷地形状（筆界）は自動取得できない。矩形近似は面積が合うだけで、境界線の位置は実際と異なる
- ハザードはタイルの配色から読むため、凡例外の配色や自治体独自の区分は判定できない。取得できなかった場合は「危険なし」ではなく警告として残る
- API キーはサーバ上の設定ファイルに平文で保存される（ファイル権限で保護）。共有サーバで使う場合は環境変数やシークレット管理の利用を検討すること
- 防火地域・日影規制・高度地区・地区計画は自動取得の対象外（すべて手入力）
