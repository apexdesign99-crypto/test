...
坪単価は**工事原価**で持ち、粗利率から請負金額を求める。工務店の見積と
施主向けの資金計画のどちらにも使えるようにするため。

```
工事原価 = 本体工事原価（原価坪単価 × 延床坪数）+ 付帯工事原価(15%) + 現場経費(5%)
請負金額 = 工事原価 ÷ (1 − 粗利率)          ← 粗利率は既定 25%
...
| 木造 | 457,000 円/坪 |
| 鉄骨造 | 580,000 円/坪 |
| 鉄筋コンクリート造 | 690,000 円/坪 |

木造の既定値は実績（延床35坪・本体工事原価1,600万円）に基づく。画面の
「本体工事原価 (円/坪)」「粗利率 (%)」、CLI/API の `unit_cost_per_tsubo` /
...
- 建築費の原価単価・料率は目安値。実際の見積は仕様・地域・時期で大きく変動する
